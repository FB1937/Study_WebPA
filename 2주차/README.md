...
- *리액트(React)**는 사용자 인터페이스(UI)를 구축하기 위한 **자바스크립트 라이브러리**.

**메타(구 페이스북)**가 2013년에 개발했고, 현재 프런트엔드 개발에서 가장 많이 사용되는 중.

특히 **SPA(Single Page Application)** 개발을 쉽게 할 수 있도록 도움.

👉 SPA는 한 개의 HTML 페이지에서 필요한 부분만 변경하는 방식.
...
리액트는 **Virtual DOM**을 사용해서 변경된 부분만 업데이트
...
리액트에서는 모든 UI를 **컴포넌트(Component)**로 나누어 관리.

컴포넌트는 **재사용 가능**하고, **독립적**으로 동작하는 UI 단위.

예를 들어, **버튼(Button) 컴포넌트**를 만들어서 여러 곳에서 사용할 수 있음.
...
일반적인 HTML 페이지에 `<script>` 태그를 추가해서 React를 사용할 수도 있음.
...
아래 명령어를 실행하면 리액트 프로젝트가 자동으로 설정.
